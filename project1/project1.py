...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    predicted = np.dot(theta,feature_vector)+theta_0
    loss = max(0.0,1-label*predicted)
    return loss
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    predicted = np.matmul(feature_matrix,theta)+theta_0
    loss = max(0.0,1-label*predicted)
    return loss.mean()
...
def perceptron_single_step_update(
    ...
    completed.
    """
    if label*(np.dot(current_theta,feature_vector)+theta_0)<=0:
...
def pegasos_single_step_update(
    ...
    if label*(np.dot(current_theta,feature_vector)+current_theta_0)<=1:
        ...
        return (mult*current_theta_0, current_theta_0 + eta*label)
...
def classifier_accuracy(
    ...
    valid_predictions = classify(val_feature_matrix, theta, theta_0)
    valid_acc = accuracy(vaild_predictions, val_labels)
    return train_accuracy, valid_acc
